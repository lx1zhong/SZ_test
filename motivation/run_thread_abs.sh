...
SZ_PATH=/home/zhongyu/sz/sz2/bin
TEST_DIR=/home/zhongyu/test
ERROR_BOUND_MODE=ABS
ERROR_BOUND_Fs="1E-4" # "1E-2 5E-3 1E-3 5E-4 1E-4 5E-5 1E-5 5E-6 1E-6 5E-7 1E-7" 
ERROR_BOUND_D="1E-7"
filesize_set_KBs="1048576 262144 65536 16384 4096 1024 256 64 16"

run(){

    thread_dir=/home/zhongyu/${ERROR_BOUND_F}
    mkdir $thread_dir
    rm -rf $thread_dir/*

    DIM=1
    TYPE=f
    output=/home/zhongyu/test/paper_test/motivation/${ERROR_BOUND_MODE}_${ERROR_BOUND_F}.txt
    echo $ERROR_BOUND_F
        ...
        do
            ...
            do
                ...
                # huffman
                        echo "===huffman===" >> $output
                    ## comp
                        # echo "$SZ_PATH"/"sz -z -$TYPE -g 0 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]}"
                        $SZ_PATH"/"sz -z -$TYPE -g 0 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]} >> $output
                ...
                # zstd
                        echo "===zstd===" >> $output
                    ## comp
                        # echo "$SZ_PATH"/"sz -z -$TYPE -g 1 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]}"
                        $SZ_PATH"/"sz -z -$TYPE -g 1 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]} >> $output
                        filesize_compressed=`ls -l $thread_dir"/"$piecex".sz" | awk '{print $5}'`
                        echo "$filesize_compressed/$filesize_real" >> $output 
                    ## decomp
                        # echo "$SZ_PATH"/"sz -x -$TYPE -s $thread_dir"/"$piecex.sz -$DIM ${dims[@]}"
                ...
                # fse
                        echo "===fse===" >> $output
                    ## comp
                        # echo "$SZ_PATH"/"sz -z -$TYPE -g 2 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]}"
                        $SZ_PATH"/"sz -z -$TYPE -g 2 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]} >> $output
                        filesize_compressed=`ls -l $thread_dir"/"$piecex".sz" | awk '{print $5}'`
                        echo "$filesize_compressed/$filesize_real" >> $output 
                    ## decomp
                        # echo "$SZ_PATH"/"sz -x -$TYPE -s $thread_dir"/"$piecex.sz -$DIM ${dims[@]}"
