...
# A dir
    ...
    do
        ...
        if [[ $num_of_pieces -lt 10 ]] # when too many pieces(pnly 26 support), use previos result
        then
            i=0
            bytes=type_len
            while [ $i -lt $DIM ]
        ...
        do
            ...
            then
                ...
                split $TEST_DIR"/"$dir"/"$file -b $bytes $TEST_DIR"/"$dir"/"piece
                

                i=0
                for piecex in `ls $TEST_DIR"/"$dir`
                do
            ...
            # huffman
                    echo "===huffman===" >> $output
                ## comp
                    # echo "$SZ_PATH"/"sz -z -$TYPE -g 0 -i $TEST_DIR"/"$dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]}"
                    $SZ_PATH"/"sz -z -$TYPE -g 0 -i $TEST_DIR"/"$dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]} >> $output
                    filesize_compressed=`ls -l $TEST_DIR"/"$dir"/"$piecex".sz" | awk '{print $5}'`
                    echo "$filesize_compressed/$filesize_real" >> $output 
                ## decomp
                    # echo "$SZ_PATH"/"sz -x -$TYPE -s $TEST_DIR"/"$dir"/"$piecex.sz -$DIM ${dims[@]}"
                    # $SZ_PATH"/"sz -x -$TYPE -s $TEST_DIR"/"$dir"/"$piecex.sz -$DIM ${dims[@]} >> $output

            # zstd
                    echo "===zstd===" >> $output
                ## comp
                    # echo "$SZ_PATH"/"sz -z -$TYPE -g 1 -i $TEST_DIR"/"$dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]}"
                    $SZ_PATH"/"sz -z -$TYPE -g 1 -i $TEST_DIR"/"$dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]} >> $output
                    filesize_compressed=`ls -l $TEST_DIR"/"$dir"/"$piecex".sz" | awk '{print $5}'`
                    echo "$filesize_compressed/$filesize_real" >> $output 
                ## decomp
                    # echo "$SZ_PATH"/"sz -x -$TYPE -s $TEST_DIR"/"$dir"/"$piecex.sz -$DIM ${dims[@]}"
            ...
            # fse
                    echo "===fse===" >> $output
                ## comp
                    # echo "$SZ_PATH"/"sz -z -$TYPE -g 2 -i $TEST_DIR"/"$dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]}"
                    $SZ_PATH"/"sz -z -$TYPE -g 2 -i $TEST_DIR"/"$dir"/"$piecex -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]} >> $output
                    filesize_compressed=`ls -l $TEST_DIR"/"$dir"/"$piecex".sz" | awk '{print $5}'`
                    echo "$filesize_compressed/$filesize_real" >> $output 
                ## decomp
                    # echo "$SZ_PATH"/"sz -x -$TYPE -s $TEST_DIR"/"$dir"/"$piecex.sz -$DIM ${dims[@]}"
                    ...
                    rm -rf  $TEST_DIR"/"$dir"/"$piecex.sz  $TEST_DIR"/"$dir"/"$piecex.sz.out
                ...
                echo rm -f $TEST_DIR"/"$dir"/piece*"
                # delete all pieces
                rm -f $TEST_DIR"/"$dir"/piece"*
