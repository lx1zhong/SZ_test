        ...
        do
            ...
            do
                ...
                # huffman
                        echo "===huffman===" >> $output
                    ## comp
                        # echo "$SZ_PATH"/"sz -z -$TYPE -g 0 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -R $ERROR_BOUND -$DIM ${dims[@]}"
                        $SZ_PATH"/"sz -z -$TYPE -g 0 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -R $ERROR_BOUND -$DIM ${dims[@]} >> $output
                ...
                # zstd
                        echo "===zstd===" >> $output
                    ## comp
                        # echo "$SZ_PATH"/"sz -z -$TYPE -g 1 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -R $ERROR_BOUND -$DIM ${dims[@]}"
                        $SZ_PATH"/"sz -z -$TYPE -g 1 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -R $ERROR_BOUND -$DIM ${dims[@]} >> $output
                        filesize_compressed=`ls -l $thread_dir"/"$piecex".sz" | awk '{print $5}'`
                        echo "$filesize_compressed/$filesize_real" >> $output 
                    ## decomp
                        # echo "$SZ_PATH"/"sz -x -$TYPE -s $thread_dir"/"$piecex.sz -$DIM ${dims[@]}"
                ...
                # fse
                        echo "===fse===" >> $output
                    ## comp
                        # echo "$SZ_PATH"/"sz -z -$TYPE -g 2 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -R $ERROR_BOUND -$DIM ${dims[@]}"
                        $SZ_PATH"/"sz -z -$TYPE -g 2 -i $thread_dir"/"$piecex -M $ERROR_BOUND_MODE -R $ERROR_BOUND -$DIM ${dims[@]} >> $output
                        filesize_compressed=`ls -l $thread_dir"/"$piecex".sz" | awk '{print $5}'`
                        echo "$filesize_compressed/$filesize_real" >> $output 
                    ## decomp
                        # echo "$SZ_PATH"/"sz -x -$TYPE -s $thread_dir"/"$piecex.sz -$DIM ${dims[@]}"
