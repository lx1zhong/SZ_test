# 压缩/解压时延随EB的变化柱状图，每个数据集一个图

# DIR--EB--FILE

from cProfile import label
import matplotlib.pyplot as plt
import matplotlib
import numpy as np
import glob2
import os
import brewer2mpl

input_files = glob2.glob("/home/zhongyu/test/paper_test/test1/3/output/[2]A*.txt")

ID = ['RATIO_PART','RATIO_ALL','CTIME_PART','CTIME_ALL','DTIME_PART','DTIME_ALL']
RATIO_PART = 0
RATIO_ALL = 1
CTIME_PART = 2
CTIME_ALL = 3
DTIME_PART = 4
DTIME_ALL = 5
TOTAL_NUM = 6


HUFFMAN = 0
ZSTD = 1
FSE = 2
PLAN_NUM = 3

No = {'bump_dense':0, 'eddy_velx_f4':1, 'EXAALT':2, 'CESM_ATM':3, 'Hurricane':4, 'Miranda':5, 'EXASKY_NYX':6, 'QMCPack':7}
# filter = dataset_names

# eb range to show
bottom = [3,1,1,2,3,3,1,1]
top    = [8,6,6,7,8,8,6,6]

def get_num(string):
    temp = string.split('=')
    temp = temp[1]
    if temp[-1] == ',':
        temp =  temp[:-1]
    return int(temp)
def get_f(string):
    temp = string.split('=')
    temp = temp[1]
    if temp[-1] == ',':
        temp =  temp[:-1]
    return float(temp)

def num2tag(x):
    if x < 1024:
        return str(x)+'K'
    elif x < 1024*1024:
        return str(int(x/1024))+'M'
    else:
        return str(int(x/1024/1024))+'G'

def cal_size(stringa, stringb):
    temp = stringa.split('x')
    n = 1
    for t in temp:
        n = n * int(t)
    n *= 4
    if 'd' in stringb:
        n = n * 2
    return int(n/1024)

for input_file in input_files:
    print(input_file)
    temp = input_file.split('/')
    temp = temp[-1]
    temp = temp.split('.')
    ebMode = temp[0]
    if ebMode[0] == '2':
        ebMode = ebMode[1:]

    f = open(input_file, 'r')


    # # 最后打印数据
    stat = [ {} for i in range(TOTAL_NUM)]
    # stats_com = {}
    # stats_decom = {}
    dataset_names = []

    file_count = 0
    error_bound = ""
    dir_flag = 0
    dirs_size = {}

    lines = f.readlines()
    i = 1
    while i <= len(lines):
        line = lines[i]
        # print(str(i) + ':' + line[:-1])
        # if 'EXAALT-2' in line or 'EXASKY_NYX' in line or 'coordinates' in line or 'bump_dense' in line or 'dpot' in line or 'eddy_velx_f4' in line :
        #     i += 1
        #     while 'DIR' not in lines[i] and 'FINISHED' not in lines[i]:
        #         i += 1
        #     continue
        if "DIR" in line:
            dir_flag = 1
            temp = line.split()
            temp = temp[1].split('-')
            # dataset name
            dir_name = temp[1]
            dir_size = cal_size(temp[2], temp[3])
            # print(dir_name + ':' + str(dir_size))
            dirs_size[dir_name] = dir_size
            # print("dir:"+dir_name)
            dataset_names.append(dir_name)

            for j in range(TOTAL_NUM):
                stat[j][dir_name] = [{},{},{}]

        elif "EB" in line:
            file_count = 0
            temp = line.split()
            error_bound = temp[-1]
            
            # stats[dir_name][size] = [0,0,0]
            t_eb = [ [0,0,0] for i in range(TOTAL_NUM)]
            huffman_ctime = 0
            zstd_ctime = 0
            fse_ctime = 0
            huffman_dtime = 0
            zstd_dtime = 0
            fse_dtime = 0

        elif "FILE" in line:
            t = [ [0,0,0] for i in range(TOTAL_NUM)]
            temp = line.split()
            file_name = temp[1]

            wrong = 0

            if i+2 < len(lines) and 'all same data!' in lines[i+2]:
                i += 2
                while 'FILE' not in lines[i] and 'EB' not in lines[i] and 'DIR' not in lines[i] and 'CNT' not in lines[i] and 'FINISHED' not in lines[i]:
                    i += 1
                continue
            
            i += 1
            # huffman
            # print(str(i) + ':' + lines[i])
            # if '===huffman===' not in lines[i]:
            #     i += 1
            #     continue
            assert('===huffman===' in lines[i])
            assert('[huffman]' in lines[i+3])
            assert('[huffman]' in lines[i+8])
            assert('[huffman]' in lines[i+13])
            temp = lines[i+3].split()
            t[CTIME_PART][HUFFMAN] += get_f(temp[-1])
            temp = lines[i+8].split()
            t[CTIME_PART][HUFFMAN] += get_f(temp[-1])
            temp = lines[i+13].split()
            t[CTIME_PART][HUFFMAN] += get_f(temp[-1])

            temp = lines[i+3+2].split()
            t[CTIME_ALL][HUFFMAN] += float(temp[-1])
            temp = lines[i+8+2].split()
            t[CTIME_ALL][HUFFMAN] += float(temp[-1])
            temp = lines[i+13+2].split()
            t[CTIME_ALL][HUFFMAN] += float(temp[-1])

            temp = lines[i+16].split('/')
            t[RATIO_ALL][HUFFMAN] += float(temp[1]) / float(temp[0])

            
            if '[decoder]' not in lines[i+17]:
                i += 17
                while 'FILE' not in lines[i] and 'EB' not in lines[i] and 'DIR' not in lines[i] and 'CNT' not in lines[i] and 'FINISHED' not in lines[i]:
                    i += 1
                continue
            assert('[decoder]' in lines[i+17])
            assert('[decoder]' in lines[i+19])
            assert('[decoder]' in lines[i+21])
            temp = lines[i+17].split()
            t[DTIME_PART][HUFFMAN] += get_f(temp[-1])
            temp = lines[i+19].split()
            t[DTIME_PART][HUFFMAN] += get_f(temp[-1])
            temp = lines[i+21].split()
            t[DTIME_PART][HUFFMAN] += get_f(temp[-1])
            
            temp = lines[i+17+1].split()
            t[DTIME_ALL][HUFFMAN] += float(temp[-2])
            temp = lines[i+19+1].split()
            t[DTIME_ALL][HUFFMAN] += float(temp[-2])
            temp = lines[i+21+1].split()
            t[DTIME_ALL][HUFFMAN] += float(temp[-2])

            if 'origin data' in lines[i+7]:
                i += 1

            i += 23
            # zstd
            assert('===zstd===' in lines[i])
            assert('[zstd]' in lines[i+2])
            assert('[zstd]' in lines[i+6])
            assert('[zstd]' in lines[i+10])
            temp = lines[i+2].split()
            t[CTIME_PART][ZSTD] += get_f(temp[-1])
            temp = lines[i+6].split()
            t[CTIME_PART][ZSTD] += get_f(temp[-1])
            temp = lines[i+10].split()
            t[CTIME_PART][ZSTD] += get_f(temp[-1])
            
            temp = lines[i+2+2].split()
            t[CTIME_ALL][ZSTD] += float(temp[-1])
            temp = lines[i+6+2].split()
            t[CTIME_ALL][ZSTD] += float(temp[-1])
            temp = lines[i+10+2].split()
            t[CTIME_ALL][ZSTD] += float(temp[-1])
            
            temp = lines[i+13].split('/')
            t[RATIO_ALL][ZSTD] += float(temp[1]) / float(temp[0])

            if '[decoder]' not in lines[i+14]:
                i += 14
                while 'FILE' not in lines[i] and 'EB' not in lines[i] and 'DIR' not in lines[i] and 'CNT' not in lines[i] and 'FINISHED' not in lines[i]:
                    i += 1
                continue
            assert('[decoder]' in lines[i+14])
            assert('[decoder]' in lines[i+16])
            assert('[decoder]' in lines[i+18])
            temp = lines[i+14].split()
            t[DTIME_PART][ZSTD] += get_f(temp[-1])
            temp = lines[i+16].split()
            t[DTIME_PART][ZSTD] += get_f(temp[-1])
            temp = lines[i+18].split()
            t[DTIME_PART][ZSTD] += get_f(temp[-1])
            
            temp = lines[i+14+1].split()
            t[DTIME_ALL][ZSTD] += float(temp[-2])
            temp = lines[i+16+1].split()
            t[DTIME_ALL][ZSTD] += float(temp[-2])
            temp = lines[i+18+1].split()
            t[DTIME_ALL][ZSTD] += float(temp[-2])

            i += 20

            # fse
            # print(str(i) + ':' + lines[i])
            assert('===fse===' in lines[i])
            assert('[fse]' in lines[i+3])
            assert('[fse]' in lines[i+8])
            assert('[fse]' in lines[i+13])
            temp = lines[i+3].split()
            t[CTIME_PART][FSE] += get_f(temp[-1])
            temp = lines[i+8].split()
            t[CTIME_PART][FSE] += get_f(temp[-1])
            temp = lines[i+13].split()
            t[CTIME_PART][FSE] += get_f(temp[-1])

            temp = lines[i+3+2].split()
            t[CTIME_ALL][FSE] += float(temp[-1])
            temp = lines[i+8+2].split()
            t[CTIME_ALL][FSE] += float(temp[-1])
            temp = lines[i+13+2].split()
            t[CTIME_ALL][FSE] += float(temp[-1])
            
            temp = lines[i+16].split('/')
            t[RATIO_ALL][FSE] += float(temp[1]) / float(temp[0])
            
            if '[decoder]' not in lines[i+17]:
                i += 17
                while 'FILE' not in lines[i] and 'EB' not in lines[i] and 'DIR' not in lines[i] and 'CNT' not in lines[i] and 'FINISHED' not in lines[i]:
                    i += 1
                continue
            assert('[decoder]' in lines[i+17])
            assert('[decoder]' in lines[i+19])
            assert('[decoder]' in lines[i+21])
            temp = lines[i+17].split()
            t[DTIME_PART][FSE] += get_f(temp[-1]) 
            temp = lines[i+19].split()
            t[DTIME_PART][FSE] += get_f(temp[-1])
            temp = lines[i+21].split()
            t[DTIME_PART][FSE] += get_f(temp[-1])
            
            temp = lines[i+17+1].split()
            t[DTIME_ALL][FSE] += float(temp[-2])
            temp = lines[i+19+1].split()
            t[DTIME_ALL][FSE] += float(temp[-2])
            temp = lines[i+21+1].split()
            t[DTIME_ALL][FSE] += float(temp[-2])

            i += 22
            
            for j in range(TOTAL_NUM):
                for k in range(PLAN_NUM):
                    t_eb[j][k] += t[j][k]

            # print('end:'+str(i) + ':' + lines[i])
            file_count += 1

        elif "CNT" in line:
            # print("dir:"+dir_name+",eb:"+error_bound+",cnt:"+str(file_count))
            # end of one eb
            if file_count == 0:
                i += 1
                continue
            
            # calculate ave time
            e = int(error_bound[-1])
            if ebMode == 'ABS' and dir_name in No.keys() and e >= bottom[No[dir_name]] and e <= top[No[dir_name]] or ebMode == 'PW_REL':
                for j in range(TOTAL_NUM):
                    for k in range(PLAN_NUM):
                        if j == RATIO_ALL:
                            stat[j][dir_name][k][error_bound] = (t_eb[j][k]/file_count)
                        elif j == CTIME_PART or j == DTIME_PART:
                            stat[j][dir_name][k][error_bound] = dirs_size[dir_name] / 2 / ((t_eb[j][k]/file_count/3) * 1000) if t_eb[j][k] != 0 else 0 
                        else:
                            stat[j][dir_name][k][error_bound] = ((t_eb[j][k]/file_count/3) * 1000)
            
        elif "FINISHED" in line:
            break 
            
        i += 1

    f.close()

    print(dataset_names)
    # matplotlib.rcParams['font.family']='Times New Roman'

    bmap = brewer2mpl.get_map('set1', 'qualitative', 9)
    colors = bmap.mpl_colors

    filter = ['bump_dense', 'eddy_velx_f4', 'EXAALT', 'CESM_ATM', 'Hurricane', 'Miranda', 'EXASKY_NYX', 'QMCPack']
    # filter = dataset_names

    # bottom = [3,2,1,3,4,3,1,1]
    # top    = [8,7,6,8,8,8,6,6]

    i = 1
    r=2
    l=4
    size = [12,5*r/2]

    # 局部压缩时间
    fig = plt.figure(figsize=size, dpi=100)
    ylims=[0,850,1350,850,550,500,1300,500,580]
    for dataset in filter:
        ax = fig.add_subplot(r,l,i)
        ax.set_title('('+ chr(ord('a')+i-1) +') '+ dataset + ' (' + num2tag(dirs_size[dataset]) + 'B)', y=-0.5, fontsize=14)
        
        ax.set_ylabel('Comp. Throughput(MB/s)')
        if 'ABS' in ebMode:  
            ax.set_xlabel('Absolute Error Bounds')
        elif 'PW_REL' in ebMode:  
            ax.set_xlabel('Point-wise Relative Error Bounds')
        
        x = np.arange(len(stat[CTIME_PART][dataset][0].keys())) #总共有几组，就设置成几，我们这里有三组，所以设置为3
        total_width, n = 0.6, 2    # 有多少个类型，只需更改n即可，比如这里我们对比了四个，那么就把n设成4
        width = total_width / n
        x = x - (total_width - width) / 2
        
        plt.grid(linestyle=':', color="gray",axis='y',zorder=1)
        c0=plt.bar(x-width,stat[CTIME_PART][dataset][HUFFMAN].values(),width=width-0.06,label='Huffman',lw=0.02,edgecolor='Black',color='w',zorder=2)
        matplotlib.rcParams['hatch.linewidth']=0.04
        c1=plt.bar(x,stat[CTIME_PART][dataset][ZSTD].values(),width=width-0.06,label='Zstd',lw=0.01,edgecolor='black',color=colors[4],zorder=2)
        c2=plt.bar(x+width,stat[CTIME_PART][dataset][FSE].values(),width=width-0.06,label='ADT-FSE',lw=0.01,edgecolor='black',color=colors[2],zorder=2)
        plt.xticks(x,stat[CTIME_PART][dataset][0].keys())
        plt.ylim([0,ylims[i]]),

        plt.legend([c0,c1,c2],['Huff','Zstd','ADT-FSE'],frameon=False,fontsize=7,loc='upper center',ncol=3,columnspacing=1) #去掉图例边框
        # plt.legend(loc='best',frameon=False) #去掉图例边框
        i += 1

    # matplotlib.rcParams['font.family']='SimHei'
    # matplotlib.rcParams['figure.figsize']=[12,8]

    plt.subplots_adjust(left=0.06,right=0.96,bottom=0.16,top=0.96,wspace=0.4,hspace=0.6) 
    # plt.tight_layout()
    # plt.legend(loc = 'upper right')
    picpath = '/home/zhongyu/test/paper_test/test1/3/fig/com_' + ebMode + '.png'
    plt.savefig(picpath)
    picpath_pdf = '/home/zhongyu/test/paper_test/test1/3/fig/com_' + ebMode + '.pdf'
    plt.savefig(picpath_pdf)
    print(picpath_pdf)
    # plt.show()

    # 局部解压时间
    i = 1
    fig2 = plt.figure(figsize=size, dpi=100)
    ylims=[0,750,1100,700,500,700,1300,550,550]
    for dataset in filter:
        # print(dataset)
        # print(stat[DTIME_PART][dataset][HUFFMAN])
        # print(stat[DTIME_PART][dataset][ZSTD])
        # print(stat[DTIME_PART][dataset][FSE])
        ax = fig2.add_subplot(r,l,i)
        ax.set_title('('+ chr(ord('a')+i-1) +') '+ dataset + ' (' + num2tag(dirs_size[dataset]) + 'B)', y=-0.5, fontsize=14)
        # if (i-1)%l==0:
        ax.set_ylabel('Decomp. Throughput(MB/s)')
        if 'ABS' in ebMode:  
            ax.set_xlabel('Absolute Error Bounds')
        elif 'PW_REL' in ebMode:  
            ax.set_xlabel('Point-wise Relative Error Bounds')
        
        x = np.arange(len(stat[DTIME_PART][dataset][0].keys())) #总共有几组，就设置成几，我们这里有三组，所以设置为3
        total_width, n = 0.6, 2    # 有多少个类型，只需更改n即可，比如这里我们对比了四个，那么就把n设成4
        width = total_width / n
        x = x - (total_width - width) / 2

        plt.grid(linestyle=':', color="gray",axis='y',zorder=1)
        c0=plt.bar(x-width,stat[DTIME_PART][dataset][HUFFMAN].values(),width=width-0.06,label='Huffman',lw=0.02,edgecolor='Black',color='w',zorder=2)
        # matplotlib.rcParams['hatch.color']=colors[0]
        matplotlib.rcParams['hatch.linewidth']=0.04
        c1=plt.bar(x,stat[DTIME_PART][dataset][ZSTD].values(),width=width-0.06,label='Zstd',lw=0.01,edgecolor='black',color=colors[4],zorder=2)
        c2=plt.bar(x+width,stat[DTIME_PART][dataset][FSE].values(),width=width-0.06,label='ADT-FSE',lw=0.01,edgecolor='black',color=colors[2],zorder=2)
        plt.xticks(x,stat[DTIME_PART][dataset][0].keys())
        plt.ylim([0,ylims[i]]),

        plt.legend([c0,c1,c2],['Huff','Zstd','ADT-FSE'],frameon=False,fontsize=7,loc='upper center',ncol=3,columnspacing=1) #去掉图例边框
        # plt.legend(loc='best',frameon=False) #去掉图例边框
        i += 1

        # 网格线
        # plt.grid(linestyle=":",lw=0.1, color="black",axis='y',zorder=0)
        # c0=plt.bar(x-width,stat[DTIME_PART][dataset][HUFFMAN].values(),width=width-0.06,label='Huffman',lw=0.02,edgecolor='Black',color='w',zorder=100)
        # # matplotlib.rcParams['hatch.color']=colors[0]
        # matplotlib.rcParams['hatch.linewidth']=0.04
        # c1=plt.bar(x,stat[DTIME_PART][dataset][ZSTD].values(),width=width-0.06,label='Zstd',lw=0.01,edgecolor=colors[0],hatch='//////',color='w')
        # plt.bar(x,stat[DTIME_PART][dataset][ZSTD].values(),width=width-0.06,label='Zstd1',lw=0.02,edgecolor='Black',color='none')
        # c2=plt.bar(x+width,stat[DTIME_PART][dataset][FSE].values(),width=width-0.06,label='ADT-FSE',lw=0.01,edgecolor=colors[2],hatch='\\\\\\\\\/////////',color='w')
        # plt.bar(x+width,stat[DTIME_PART][dataset][FSE].values(),width=width-0.06,label='ADT-FSE1',lw=0.02,edgecolor='Black',color='none')
        
        # plt.xticks(x,stat[DTIME_PART][dataset][0].keys())
        # plt.ylim([0,ylims[i]])

    # matplotlib.rcParams['font.family']='SimHei'
    # matplotlib.rcParams['figure.figsize']=[12,8]

    plt.subplots_adjust(left=0.06,right=0.96,bottom=0.16,top=0.96,wspace=0.4,hspace=0.6) 
    # plt.tight_layout()
    # plt.legend(loc = 'upper right')
    picpath = '/home/zhongyu/test/paper_test/test1/3/fig/decom_' + ebMode + '.png'
    plt.savefig(picpath)
    picpath_pdf = '/home/zhongyu/test/paper_test/test1/3/fig/decom_' + ebMode + '.pdf'
    plt.savefig(picpath_pdf)
    print(picpath_pdf)



from matplotlib import font_manager
 
# for font in font_manager.fontManager.ttflist:
#     # 查看字体名以及对应的字体文件名
#     print(font.name, '-', font.fname)