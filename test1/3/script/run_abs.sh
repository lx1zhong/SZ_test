...
# A dir
    ...
    do  
        ...
        # zstd
                echo "===zstd===" >> $output
            ## comp
                # echo "$SZ_PATH"/"sz -z -$TYPE -g 1 -i $TEST_DIR"/"$dir"/"$file -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]}"
                $SZ_PATH"/"sz -z -$TYPE -g 1 -i $TEST_DIR"/"$dir"/"$file -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]} >> $output
                $SZ_PATH"/"sz -z -$TYPE -g 1 -i $TEST_DIR"/"$dir"/"$file -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]} >> $output
                $SZ_PATH"/"sz -z -$TYPE -g 1 -i $TEST_DIR"/"$dir"/"$file -M $ERROR_BOUND_MODE -A $ERROR_BOUND -$DIM ${dims[@]} >> $output
                filesize_compressed=`ls -l $TEST_DIR"/"$dir"/"$file".sz" | awk '{print $5}'`
                echo "$filesize_compressed/$max" >> $output 
